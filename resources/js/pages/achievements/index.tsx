...
export default function AchievementsIndex({ earnedAchievements, availableAchievements, badges, trophies, stats, categories }: Props) {
    const [search, setSearch] = useState('');
    const [selectedCategory, setSelectedCategory] = useState('all');
    const [selectedStatus, setSelectedStatus] = useState('all');
    
    const allAchievements = [...earnedAchievements, ...availableAchievements];

    // Filtrer les achievements localement
    const filteredAchievements = allAchievements.filter(achievement => {
        ...
        const matchesStatus = selectedStatus === 'all' || 
            (selectedStatus === 'earned' && achievement.earned_at) ||
            (selectedStatus === 'available' && !achievement.earned_at);
                    ...
                    <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                        <Card>
                            <CardContent className="p-4 text-center">
                                ...
                                <div className="text-2xl font-bold text-yellow-600">{stats.total_earned}</div>
                        ...
                        <Card>
                            <CardContent className="p-4 text-center">
                                ...
                                <div className="text-2xl font-bold text-green-600">{stats.completion_percentage}%</div>
                        ...
                        <Card>
                            <CardContent className="p-4 text-center">
                                ...
                                <div className="text-2xl font-bold text-purple-600">#{stats.rank?.rank || 'N/A'}</div>
                            ...
                            >
                                ...
                                <CardContent className="space-y-4">
                                    ...
                                    <div className="space-y-2 text-center">
                                        <div className={`text-4xl ${!achievement.earned_at ? 'grayscale' : ''}`}>{achievement.icon}</div>
                                        <div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="grid grid-cols-2 gap-4 text-center md:grid-cols-4">
                                {categories.map((cat) => {
                                    ...
                                    const unlockedCount = categoryAchievements.filter((a) => a.earned_at).length;
                                    const percentage =
